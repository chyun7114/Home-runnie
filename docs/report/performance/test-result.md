...
본 시나리오에서 timeout 8초는 전체 세션이 아니라 각 단계(auth/join/message)의 최대 대기 시간이다.

10~15 iters/s 구간에서는 p95/p99가 수백 ms 수준으로 사용자 체감상 즉각적인 응답이 가능했다.

반면 20 iters/s부터는 p95가 2~3초 수준으로 증가하여, timeout 이전에 이미 사용자 경험이 크게 저하되는 구간에 진입했다.

따라서 시스템의 붕괴는 timeout 도달 시점이 아니라, latency가 수 초 단위로 증가하는 시점(≈20 iters/s)부터 시작된다고 판단할 수 있다.
